...
class AldeiaService
{
    ...
    const CACHE_RETRY_DELAY = 100; // milliseconds
    ...
    public function getAllAldeias(): Collection
    {
        ...
        return $this->getWithCacheRecovery($cacheKey, function () {
            return Aldeia::select(['idAldeia', 'nome', 'nm_uf'])
            ->orderBy('nome')
            ->get()
            ->map(function ($aldeia) {
                return [
    ...
    private function executeWithFallback(string $cacheKey, callable $callback, Throwable $originalException)
    {
        try {
            ...
            throw new Exception(
                "Falha no cache e no fallback para {$cacheKey}: " .
    ...
    public function clearCache(?string $specificKey = null): bool
    {
        try {
            if ($specificKey) {
                ...
                Log::info("Cache específico limpo: {$cacheKey}", ['success' => $deleted]);
                return $deleted;
            } else {
